...
FTP.prototype._createWorker = function(onSuccess) {
  ...
  if (this.options.secure) {
    for (var i in this.options.secureOptions) {
      secureOptions[i] = this.options.secureOptions[i];
    }
  ...
  socket.once('close', function(had_err) {
    ondone(id);
    if (!self._sockets) {
			clearTimeout(self._multiSocketHandler);
...
FTP.prototype.pwd = function(cb) { // PWD is optional
  ...
  this._send('PWD', function(err, text, code) {
    if (code === 502) {
      ...
      return cb(err);
    cb(undefined, RE_WD.exec(text) ? RE_WD.exec(text)[1] : null);
  });
};
  ...
  this._send('PASV', function reentry(err, text) {
    ...
    self._pasvConnect(id, ip, port, function(err, sock) {
      if (err) {
        ...
        // the Internet
        if (self._worker[id] && self._worker[id].socket && ip !== self._worker[id].socket.remoteAddress) {
